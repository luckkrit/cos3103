...
client = MongoClient('mongodb://localhost:27017/', document_class=RawBSONDocument)
db = client['classic']
raw_customers = db['customers2']
print("Reading raw BSON documents:")
for raw_customer in raw_customers.find():
    print(f"Raw BSON: {raw_customer.raw}")
    print(f"BSON size: {len(raw_customer.raw)} bytes")
    print(f"BSON hex: {raw_customer.raw.hex()}")
    # Convert back to dict when needed
    decoded = bson.decode(raw_customer.raw)
...
- Insert document using BSON

```python

doc_dict = {    "name": "John Doe","age": 30,"email": "john@example.com",}
# Convert to BSON bytes
bson_bytes = bson.encode(doc_dict)
print(f"BSON size: {len(bson_bytes)} bytes")
print(f"BSON hex: {bson_bytes.hex()}")
# Decode and insert (MongoDB will auto-generate _id)
decoded_doc = bson.decode(bson_bytes)
print(f"Document before insert: {decoded_doc}")
result = users.insert_one(decoded_doc)
print(f"Inserted with auto-generated _id: {result.inserted_id}")
# Verify the document
inserted_doc = users.find_one({"_id": result.inserted_id})
print(f"Document in database: {inserted_doc}")
```
